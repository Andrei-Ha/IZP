...
public partial class Archive_PES : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            string commandStringYear = "SELECT DISTINCT TO_CHAR(DATE_IN,'yyyy') as DATE_IN FROM PES_IZP_BOOK ORDER BY DATE_IN DESC";
    ...
    protected void Func_Build_Grid1()
    {
        string str_dop = "";
        string str_term = "";
        string str_podr = "для всех подразделений Пинских ЭС";
        ...
        if (DropDownListOrg.SelectedValue != "0")
        {
            ...
            str_podr = "для подразделения " + DropDownListOrg.SelectedItem.Text;
        }
        ...
        {
            str_dop += " AND PES_IZP_BOOK.K_GROUP = " + DropDownListGroup.SelectedValue;
        }
        string commandString = "SELECT count(FAC_NUM) FROM PES_IZP_BOOK WHERE" + str_dop;
        OleDbCommand myCommand = new OleDbCommand(commandString, myConnection);
        string commandStringBrak = "SELECT count(FAC_NUM) FROM PES_IZP_BOOK WHERE" + str_dop + " AND RESULT_EXP=0";
        ...
        str_dop = " AND" + str_dop;
        SqlDataSource1.SelectCommand = "SELECT NUM_P,NAME_ORG,NAME_BRIG,NAME_GROUP,IZP_SPR_PROTECT.NAME,FAC_NUM, TO_CHAR(DATE_IN,'DD.MM.YYYY') as DATE_IN,TO_CHAR(DATE_EXP,'DD.MM.YYYY') as DATE_EXP,V_PROT,V_ISOL,V_WORK,V_WIRE,V_INDIC,C_RIGHT,C_LEFT,C_POINTER,T_WATER,T_AIR,HUMIDITY,PRESSURE,Replace(REPLACE(RESULT_EXP,'0','негодно'),'1','годно') as RESULT_EXP, TO_CHAR(DATE_IN,'DD.MM.YYYY') as DATE_PROT,NUM_ACT,NOTE FROM PES_IZP_BOOK,PES_IZP_SPR_ORG,PES_IZP_SPR_BRIG,IZP_SPR_PERIOD,IZP_SPR_PROTECT WHERE PES_IZP_BOOK.K_GROUP=NUM_PROTCL AND K_ORGAN=PES_IZP_SPR_ORG.CODE AND K_BRIG=PES_IZP_SPR_BRIG.CODE ANd IZP_SPR_PROTECT.ID_=K_NAME" + str_dop + " order by NUM_P ";
        SqlDataSource2.SelectCommand = "Select K_GROUP,NAME_GROUP, COUNT(FAC_NUM) as ALL_COUNT, (COUNT(FAC_NUM)-SUM(RESULT_EXP)) as BRAK_COUNT from pes_izp_book,IZP_SPR_PERIOD WHERE PES_IZP_BOOK.K_GROUP=NUM_PROTCL AND DATE_PROT IS NOT NULL" + str_dop + " Group by k_group,NAME_GROUP ORDER BY K_GROUP" ;
