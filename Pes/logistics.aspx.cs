...
{
    string st_filter_org = "", st_org_val = "", st_filter_brig = "", st_brig_val = "", st_filter_group = "", st_group_val = "", sr_bind_cell="";
    string st_fac_num, st_cell;
    ...
    protected void RadGrid2_ItemDataBound(object sender, Telerik.WebControls.GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            st_k_group = st_k_gr;
            st_k_organ = st_k_org;
            /*CheckBox checkbox_ES = e.Item.FindControl("check_Bind_Cell") as CheckBox;
            ...
            Label_Fac_Num.Text = Convert.ToString(DataBinder.Eval(e.Item.DataItem, "FAC_NUM"));
        }
    }
    protected void ButtonSave_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridItem item in RadGrid2.MasterTableView.Items)
        {
            ...
            TableCell cell = dataitem["BIND_CELL"];
            CheckBox checkBox = (CheckBox)cell.FindControl("check_Bind_Cell");
            TableCell cell2 = dataitem["CELL"];
            TextBox textbox = (TextBox)cell2.FindControl("TextBox_CELL");
            if (checkBox.Checked) {
                st_cell = textbox.Text; 
                string commandString = "UPDATE PES_IZP_BOOK SET CELL=" + st_cell + " WHERE ID_=" + dataitem.GetDataKeyValue("ID").ToString();
                OleDbCommand myCommand = new OleDbCommand(commandString, myConnection);
    ...
    protected void ButtonFilter_Click(object sender, EventArgs e)
    {
        ...
        //SqlDataSource2.SelectCommand = "SELECT PES_IZP_BOOK.ID_ AS ID,K_ORGAN,K_BRIG,NAME_ORG,NAME_BRIG,PES_IZP_BOOK.K_GROUP,NAME_GROUP,K_NAME,NAME,FAC_NUM, DATE_PROT,EXEC_SIGN FROM PES_IZP_BOOK,PES_IZP_SPR_ORG,PES_IZP_SPR_BRIG,IZP_SPR_PERIOD,IZP_SPR_PROTECT WHERE K_ORGAN=PES_IZP_SPR_ORG.CODE AND K_BRIG=PES_IZP_SPR_BRIG.CODE AND PES_IZP_BOOK.K_GROUP=NUM_PROTCL AND K_NAME=IZP_SPR_PROTECT.ID_ AND DATE_PROT IS NOT NULL AND EXEC_SIGN=0 " + st_filter_org + st_filter_brig + st_filter_group + "ORDER BY PES_IZP_BOOK.K_GROUP";
        SqlDataSource2.SelectCommand = "SELECT PES_IZP_BOOK.ID_ AS ID,K_ORGAN,K_BRIG,NAME_ORG,NAME_BRIG,PES_IZP_BOOK.K_GROUP,NAME_GROUP,K_NAME,NAME,CELL,FAC_NUM, DATE_PROT,EXEC_SIGN FROM PES_IZP_BOOK,PES_IZP_SPR_ORG,PES_IZP_SPR_BRIG,IZP_SPR_PERIOD,IZP_SPR_PROTECT WHERE K_ORGAN=PES_IZP_SPR_ORG.CODE AND K_BRIG=PES_IZP_SPR_BRIG.CODE AND PES_IZP_BOOK.K_GROUP=NUM_PROTCL AND K_NAME=IZP_SPR_PROTECT.ID_ AND EXEC_SIGN=0 " + st_filter_org + st_filter_brig + st_filter_group + sr_bind_cell +"ORDER BY PES_IZP_BOOK.K_GROUP";
