...
public partial class give : System.Web.UI.Page
{
    ...
    protected void ButtonSave_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridItem item in RadGrid2.MasterTableView.Items)
        {
            ...
            {
                st_ES = "1"; 
                string commandString = "UPDATE PES_IZP_BOOK SET CELL=0, EXEC_SIGN=" + st_ES + " WHERE ID_=" + dataitem.GetDataKeyValue("ID").ToString();
                OleDbCommand myCommand = new OleDbCommand(commandString, myConnection);
    ...
    protected void ButtonFilter_Click(object sender, EventArgs e)
    {
        ...
        SqlDataSource2.SelectCommand = "SELECT PES_IZP_BOOK.ID_ AS ID,K_ORGAN,K_BRIG,NAME_ORG,NAME_BRIG,PES_IZP_BOOK.K_GROUP,NAME_GROUP,K_NAME,NAME,FAC_NUM,CELL,DATE_PROT,EXEC_SIGN FROM PES_IZP_BOOK,PES_IZP_SPR_ORG,PES_IZP_SPR_BRIG,IZP_SPR_PERIOD,IZP_SPR_PROTECT WHERE K_ORGAN=PES_IZP_SPR_ORG.CODE AND K_BRIG=PES_IZP_SPR_BRIG.CODE AND PES_IZP_BOOK.K_GROUP=NUM_PROTCL AND K_NAME=IZP_SPR_PROTECT.ID_ AND DATE_PROT IS NOT NULL AND CELL<>0 AND EXEC_SIGN=0 " + st_filter_org + st_filter_brig + st_filter_group + "ORDER BY PES_IZP_BOOK.K_GROUP";
