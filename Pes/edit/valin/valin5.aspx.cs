...
public partial class valin5 : System.Web.UI.Page
{
    ...
    protected void UpdateFun()
    {
        ...
        sv_wire = ",V_WIRE=" + HiddenField_v_wire.Value;
        if (RadioButton1.Checked == true && RadioButton3.Checked == true && RadioButton5.Checked == true)
        {
            sv_indic = ",V_INDIC=" + v_indic.Text.Replace(",", ".");
            sv_indic2 = ",V_INDIC2=" + v_indic2.Text.Replace(",", ".");

            if (Convert.ToDouble(v_indic2.Text.Replace(".", ",")) <= 2.5 && Convert.ToDouble(v_indic.Text.Replace(".", ",")) >= 12.7) 
            {
                sresult_exp = "1";
            }
        }
        else
        {
            ...
            sv_indic2 = ",V_INDIC2=NULL";
        }
        Save.Enabled = false;
        Cancel.Enabled = false;
        string commandStringTEC = "UPDATE PES_IZP_BOOK SET DATE_EXP='" + RadDatePicker_date_exp.SelectedDate.ToString().Substring(0, 10) + "'" + sv_prot + sv_isol + sv_work + sv_wire + sv_indic + sv_indic2 + sc_left + sc_right + sc_pointer + ",RESULT_EXP=" + sresult_exp +
    ...
    {
        if (RadioButton2.Checked == true || RadioButton4.Checked || RadioButton6.Checked)
        {
            v_indic.Enabled = false;
            v_indic.Attributes.Add("style", "background-color:#C0C0C0");
            RFV_v_indic.Enabled = false;
            v_indic2.Enabled = false;
            v_indic2.Attributes.Add("style", "background-color:#C0C0C0");
            RFV_v_indic2.Enabled = false;
            Save.Focus();
        }
        else
        {
            ...
            v_indic2.Enabled = true;
            v_indic2.Attributes.Add("style", "background-color:White");
            RFV_v_indic2.Enabled = true;
        }
    }
}