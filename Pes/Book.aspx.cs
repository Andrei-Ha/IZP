...
public partial class Book : System.Web.UI.Page
{
    ...
    protected void RadGrid2_ItemDataBound(object sender, Telerik.WebControls.GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            Label Label_Cell = e.Item.FindControl("Label_Cell") as Label;
            Label_Cell.ForeColor = System.Drawing.Color.Green;
            Label_Cell.Text = DataBinder.Eval(e.Item.DataItem, "CELL").ToString() != "0" ? "(" + (DataBinder.Eval(e.Item.DataItem, "CELL").ToString()) + ")" : "-";
    ...
    protected void RadGrid1_ItemCommand(object source, Telerik.WebControls.GridCommandEventArgs e)
    {
        if (e.CommandName == "RowClick" && e.Item is GridDataItem)
        {
            ...
            Button_Spisan.Visible = false;
            SqlDataSource2.SelectCommand = "SELECT PES_IZP_BOOK.ID_ as ID_,NUM,NUM_P,YEAR,PERIOD,NAME_ORG,K_BRIG,NAME_BRIG,PES_IZP_BOOK.K_GROUP,IZP_SPR_PROTECT.NAME,FAC_NUM,TO_CHAR(DATE_IN,'DD.MM.YYYY') as DATE_IN,TO_CHAR(DATE_EXP,'DD.MM.YYYY') as DATE_EXP,V_PROT,V_ISOL,V_WORK,V_WIRE,V_INDIC,V_INDIC2,C_LEFT,C_RIGHT,C_POINTER,T_WATER,T_AIR,HUMIDITY,"/*PRESSURE,*/+ "RESULT_EXP,FIO_EXEC,EXEC_SIGN,DATE_PROT,NUM_ACT,NOTE,CELL FROM IZP_SPR_PERIOD,PES_IZP_BOOK,PES_IZP_SPR_ORG,PES_IZP_SPR_BRIG,IZP_SPR_PROTECT WHERE IZP_SPR_PERIOD.NUM_PROTCL=PES_IZP_BOOK.K_GROUP AND K_ORGAN=PES_IZP_SPR_ORG.CODE AND K_BRIG=PES_IZP_SPR_BRIG.CODE AND K_NAME=IZP_SPR_PROTECT.ID_ AND YEAR=" + s_year + " ORDER BY NUM_P,FAC_NUM";
            RadGrid2.DataBind();
        }
    }
    ...
    protected override void RaisePostBackEvent(IPostBackEventHandler sourceControl, string eventArgument)
    {
        ...
        if (sourceControl is RadGrid)
        {
            if (eventArgument == "RebindAndNavigate2")
            {
                ...
                SqlDataSource2.SelectCommand = "SELECT PES_IZP_BOOK.ID_ as ID_,NUM,NUM_P,YEAR,PERIOD,NAME_ORG,K_BRIG,NAME_BRIG,PES_IZP_BOOK.K_GROUP,IZP_SPR_PROTECT.NAME,FAC_NUM,TO_CHAR(DATE_IN,'DD.MM.YYYY') as DATE_IN,TO_CHAR(DATE_EXP,'DD.MM.YYYY') as DATE_EXP,V_PROT,V_ISOL,V_WORK,V_WIRE,V_INDIC,V_INDIC2,C_LEFT,C_RIGHT,C_POINTER,T_WATER,T_AIR,HUMIDITY,"/*PRESSURE,*/+ "RESULT_EXP,FIO_EXEC,EXEC_SIGN,DATE_PROT,NUM_ACT,NOTE,CELL FROM IZP_SPR_PERIOD,PES_IZP_BOOK,PES_IZP_SPR_ORG,PES_IZP_SPR_BRIG,IZP_SPR_PROTECT WHERE IZP_SPR_PERIOD.NUM_PROTCL=PES_IZP_BOOK.K_GROUP AND K_ORGAN=PES_IZP_SPR_ORG.CODE AND K_BRIG=PES_IZP_SPR_BRIG.CODE AND K_NAME=IZP_SPR_PROTECT.ID_ AND YEAR=" + HiddenFieldYEAR.Value + " ORDER BY NUM_P,FAC_NUM";
                RadGrid2.DataBind();
