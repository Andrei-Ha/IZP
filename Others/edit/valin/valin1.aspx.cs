...
public partial class valin1 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            case "1":
                RadioButton1.Text = "6кВ";
                HiddenField_v_prot.Value = "6";
                i_maxcurr = 6;
                break;
            case "2":
                RadioButton1.Text = "15кВ";
                HiddenField_v_prot.Value = "15";
                i_maxcurr = 7.5;
                break;
            case "3":
                RadioButton1.Text = "3,5кВ";
                HiddenField_v_prot.Value = "3.5";
                i_maxcurr = 2;
                break;
        }
        c_right.Focus();
    ...
    protected void UpdateFun()
    {
        ...
        if (RadioButton1.Checked == true)
        {
            ...
            sc_left = ",C_LEFT=" + c_left.Text.Replace(",", ".");
            sc_right = ",C_RIGHT=" + c_right.Text.Replace(",", ".");
            if ((Convert.ToDouble(c_left.Text.Replace(".", ",")) <= i_maxcurr) && (Convert.ToDouble(c_right.Text.Replace(".", ",")) <= i_maxcurr)) { sresult_exp = "1"; }
        }
        else
        {
            sv_prot = ",V_PROT=0";
            sc_left = ",C_LEFT=NULL";
            sc_right = ",C_RIGHT=NULL";
        }
        Save.Enabled = false;
        ...
        /*string commandStringTEC = "UPDATE IZP_BOOK SET DATE_EXP=DATE_IN" + sv_prot + sv_isol + sv_work + sv_wire + sv_indic + sc_left + sc_right + sc_pointer +",RESULT_EXP="+sresult_exp+
            ",T_WATER="+t_water+",T_AIR="+t_air+",HUMIDITY="+humidity+",PRESSURE='"+pressure+"',FIO_EXEC='"+fio_exec+"' WHERE ID_=" + id_;*/
    ...
    protected void RBCheck()
    {
        ...
        {
            c_left.Enabled = false;
            c_left.Attributes.Add("style", "background-color:#C0C0C0");
            c_right.Enabled = false;
            c_right.Attributes.Add("style", "background-color:#C0C0C0");
            RFV_c_left.Enabled = false;
            RFV_c_right.Enabled = false;
            Save.Focus();
        }
        else
        {
            c_left.Enabled = true;
            c_left.Attributes.Add("style", "background-color:White");
            c_left.Focus();
            c_right.Enabled = true;
            c_right.Attributes.Add("style", "background-color:White");
            RFV_c_left.Enabled = true;
            RFV_c_right.Enabled = true;
        }
    }
}